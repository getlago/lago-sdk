...
module LagoAPI
  class InvoicesApi
    ...
    # @param [Hash] opts the optional parameters
    # @return [Array<(Invoice, Integer, Hash)>] Invoice data, response status code and response headers
    def void_invoice_with_http_info(lago_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: InvoicesApi.void_invoice ...'
      end
      # verify the required parameter 'lago_id' is set
      if @api_client.config.client_side_validation && lago_id.nil?
        fail ArgumentError, "Missing the required parameter 'lago_id' when calling InvoicesApi.void_invoice"
      end
      # resource path
      local_var_path = '/invoices/{lago_id}/void'.sub('{' + 'lago_id' + '}', CGI.escape(lago_id.to_s))
      ...
      new_options = opts.merge(
        :operation => :"InvoicesApi.void_invoice",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
