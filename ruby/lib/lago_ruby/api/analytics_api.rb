...
module LagoAPI
  class AnalyticsApi
    ...
    def find_all_invoice_collections(opts = {})
      data, _status_code, _headers = find_all_invoice_collections_with_http_info(opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [Currency] :currency The currency of revenue analytics. Format must be ISO 4217.
    # @return [Array<(InvoiceCollections, Integer, Hash)>] InvoiceCollections data, response status code and response headers
    def find_all_invoice_collections_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: AnalyticsApi.find_all_invoice_collections ...'
      end
      # resource path
      local_var_path = '/analytics/invoice_collection'
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'InvoiceCollections'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['bearerAuth']

      new_options = opts.merge(
        :operation => :"AnalyticsApi.find_all_invoice_collections",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: AnalyticsApi#find_all_invoice_collections\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
