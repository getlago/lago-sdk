  ...
  class CreditObject
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LagoAPI::CreditObject` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LagoAPI::CreditObject`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'before_taxes')
        self.before_taxes = attributes[:'before_taxes']
      else
        self.before_taxes = nil
      end
    ...
    def list_invalid_properties
      ...
      if @before_taxes.nil?
        invalid_properties.push('invalid value for "before_taxes", before_taxes cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @before_taxes.nil?
    ...
    def hash
      [lago_id, amount_cents, amount_currency, before_taxes, item, invoice].hash
    end
