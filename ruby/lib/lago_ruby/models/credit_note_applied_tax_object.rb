...
module LagoAPI
  class CreditNoteAppliedTaxObject
    # Unique identifier of the credit note, created by Lago.
    attr_accessor :lago_credit_note_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LagoAPI::CreditNoteAppliedTaxObject` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LagoAPI::CreditNoteAppliedTaxObject`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'lago_credit_note_id')
        self.lago_credit_note_id = attributes[:'lago_credit_note_id']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          lago_credit_note_id == o.lago_credit_note_id &&
    ...
    def hash
      [lago_credit_note_id, base_amount_cents, lago_id, lago_tax_id, tax_name, tax_code, tax_rate, tax_description, amount_cents, amount_currency, created_at].hash
    end
